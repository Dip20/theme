...
public function insert_edit_uom($post)
    ...
    public function search_sale_delivery($post)
    {
        ...
        $builder->where('gl.name','Sundry Debtors');
    ...
    // public function get_bank_account_data($post)
    // {
    //     $db = $this->db;
    //     $db->setDatabase(session('DataSource')); 
    //     $builder = $db->table('account acc');
    ...
    public function UpdateData($post) {
       ...
       if ($post['type'] == 'Status') {
            ...
            if ($post['method'] == 'itemgrp') {
                $gnmodel = new GeneralModel();
                $result = $gnmodel->update_data_table('item_group', array('id' => $post['pk']), array('status' => $post['val']));
            }
